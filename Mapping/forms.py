...
class AcademicyearClassRelationForm(forms.ModelForm):
    ...
    section_id=forms.ModelMultipleChoiceField(queryset=Sections.objects.all(), widget=FilteredSelectMultiple("Sections", is_stacked=False))
#     academic_year1 = forms.ModelChoiceField(queryset = AcademicYear ,widget=forms.Select(attrs={'class':'form-control' , 'autocomplete': 'off'}))
#     class_id1 = forms.ModelChoiceField(queryset=ClassDetails.objects.all(), widget=forms.Select(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    
    class Meta:
        model =AcademicyearClassRelation
        fields = ['class_id','academic_year','section_id']
 
 
 
class ClassSectionRelationForm(forms.ModelForm):
    ClassDetail = ClassDetails.objects.all()
    class_name = forms.ModelMultipleChoiceField(queryset=ClassDetails.objects.all(), widget=FilteredSelectMultiple("ClassDetails", is_stacked=False))
...
class ClassSubjectMappingForm(forms.ModelForm):
    ClassDetail = ClassDetails.objects.all()
    class_name = forms.ModelMultipleChoiceField(queryset = ClassDetail ,widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    ...
    section_name=forms.ModelMultipleChoiceField(queryset=Sections.objects.all(), widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    
    class Meta:
        model = ClassSubjectMapping
        fields = ['class_name','subject_name','section_name']
...
class TeacherSubjectRelationForm(forms.ModelForm):
    ...
    subject_name = forms.ModelMultipleChoiceField(queryset=Subject.objects.all(), widget=FilteredSelectMultiple("Subject", is_stacked=False))
    #assistant_teacher_name = forms.ModelMultipleChoiceField(required=False,queryset=StaffDetails.objects.filter(role_type=15), widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    class_name = forms.ModelMultipleChoiceField(queryset=ClassDetails.objects.none(), widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    section_name = forms.ModelMultipleChoiceField(required=False,queryset=Sections.objects.none(), widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    ...
    def __init__(self,  *args,  **kwargs):
        super(TeacherSubjectRelationForm, self).__init__(*args, **kwargs) 
        self.fields['class_name'].queryset = StaffDetails.objects.none()
        self.fields['section_name'].queryset = Sections.objects.none()
        if self.is_bound:
            self.fields['class_name'].queryset = ClassDetails.objects.all() 
            self.fields['section_name'].queryset = Sections.objects.all()
 
class SupervisorClassRelationForm(forms.ModelForm):
    StaffDetail = StaffDetails.objects.filter(role_type = 3)  
    supervisor_name = forms.ModelMultipleChoiceField(queryset = StaffDetail ,widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
...
class ClassTeacherRelationForm(forms.ModelForm):
    ClassDetail = ClassDetails.objects.none()
    ...
    teacher_name = forms.ModelMultipleChoiceField(queryset=StaffDetails.objects.filter(role_type=2), widget=FilteredSelectMultiple("StaffDetails", is_stacked=False))
    assistant_teacher_name = forms.ModelMultipleChoiceField(required=False,queryset=StaffDetails.objects.filter(role_type=15), widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    section_name = forms.ModelMultipleChoiceField(required=False,queryset=Sections.objects.none(), widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    academic_year = forms.ModelMultipleChoiceField(queryset=AcademicDetails.objects.all(), widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    
    class Meta:
        model = ClassTeacherMapping
        fields = ['class_name','teacher_name','assistant_teacher_name']      
        
    def __init__(self,  *args,  **kwargs):
        super(ClassTeacherRelationForm, self).__init__(*args, **kwargs) 
        self.fields['class_name'].queryset = ClassDetails.objects.none()
        self.fields['section_name'].queryset = Sections.objects.none()
        if self.is_bound:
            self.fields['class_name'].queryset = ClassDetails.objects.all()   
            self.fields['section_name'].queryset = Sections.objects.all()      
