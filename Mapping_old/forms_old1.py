...
class TeacherSubjectRelationForm(forms.ModelForm):
    ...
    teacher_name = forms.MultipleChoiceField(choices=StaffDetail, widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    subject_name = forms.ModelMultipleChoiceField(queryset=Subject.objects.all(), widget=FilteredSelectMultiple("Subject", is_stacked=False))
    assistant_teacher = forms.ModelMultipleChoiceField(required=False,queryset=StaffDetails.objects.filter(role_type=15), widget=forms.SelectMultiple(attrs={'class':'form-control' , 'autocomplete': 'off'}))
    ...
    class Meta:
        ...
        fields = ['subject_name','teacher_name','assistant_teacher']
