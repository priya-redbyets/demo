...
#             print ("form is valid")
            ...
            return render(request, "class.html" ,{'ClassDetailsForm':ClassDetails_obj, 'academicyr':academicyr }) 
...
#     print "new list",new_list1
    #return render(request, "class_update.html" ,{'classdetails1':classdetails1}) 
    return render(request, "class_update.html" ,{'ClassDetailsForm':ClassDetails_obj , 'academicyr':academicyr,'Subjectlist':Subjectlist,'section1':section, "subject":new_list,"section":new_list1}) 
...
#             print ("form is valid")
            ...
            if(StaffDetails.objects.filter(email= StaffDetails_obj.email).count() > 0):
                messages.warning(request,"Email id already exist",fail_silently=True)
#                 print 'email already exist'
                
#                 StaffDetail = StaffDetailsForm()
                schools= SchoolDetailsForm()
                return render(request, "staff_registration.html" ,{'StaffDetailsForm':StaffDetail, 'schools':schools})
...
#             messages.success(request, "One record added successfully",fail_silently=True)
            return redirect('/school/view_staff/')
        else:
            StaffDetail = StaffDetailsForm()
            schools= SchoolDetailsForm()
            messages.error(request, "Invalid details please enter valid data",fail_silently=True)
            return render(request, "staff_registration.html" ,{'StaffDetailsForm':StaffDetail, 'schools':schools})
